...
LETTERS:
	...
	LETC: ; store time in station_time, if more than 10 display set to max 10


	LETD: ; delete, reduces length only if length is non-zero, shifts cursor left, only for double-press A not for confirmed letters
	onebyte_to_register LENGTH, temp
	CPI temp, 1
	BRLT END_LETTERS

	do_lcd_command 0b00010000 ;shift cursor left
	subtract_from_onebyte LENGTH, 1
